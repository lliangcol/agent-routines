...
| `desktop-design-system` | 面向桌面生产力 UI 的设计系统、tokens、密集表格、状态标记、主题 tokens 和无 AI 味视觉标准。 | `doc-check`, `gate-check` | 桌面操作台 UI 需要现代视觉规则、主题 tokens 或组件级视觉 review。 |
| `desktop-packaging-release` | Electron 桌面应用在 Windows、macOS 和 Linux 上的打包与发布就绪检查。 | `node-workspace-check`, `release-check`, `security-check`, `gate-check` | 桌面安装包、签名、产物或发布就绪度需要 dry-run 证据和策略门禁。 |
| `desktop-qa` | Electron 应用的跨平台桌面 UI QA，覆盖 renderer 截图、原生窗口行为、shell 前置条件和任务日志。 | `runtime-check`, `node-workspace-check`, `gate-check` | Electron UI 行为需要 Windows、macOS、Linux、主题、语言或原生集成测试。 |
| `dms-repair` | 数据库修复先只读确认，准备最小 SQL，并在人类 DMS 执行后只读复核。 | `db-read` | 数据库修复需要人工执行 SQL 和本地只读证据。 |
| `electron-app-builder` | Electron 应用实现，覆盖安全 main/preload/renderer 边界、白名单 IPC、本地命令执行、主题和 i18n。 | `node-workspace-check`, `runtime-check`, `security-check`, `gate-check` | 本地 Electron 操作台需要实现或架构 review。 |
...
| `i18n-checklist` | 简体中文和英文 UI 本地化检查，覆盖翻译 key、文本膨胀、状态标签稳定性和语言切换 QA。 | `doc-check`, `gate-check` | UI 语言切换或双语文本需要 key 覆盖、术语和截断检查。 |
...
| `doc-check` | 聚合文档检查和可选安全自定义命令，并明确报告缺失 runtime。 | `.\workflows\doc-check\doc-check.ps1 -Path .` | `./workflows/doc-check/doc-check.sh --path .` | `desktop-design-system`, `i18n-checklist`, `knowledge-drift`, `pay-docs` |
...
| `gate-check` | 运行安全通用门禁，如 git diff checks 和明确提供的非破坏性自定义命令。 | `.\workflows\gate-check\gate-check.ps1 -Path .` | `./workflows/gate-check/gate-check.sh --path .` | `api-sync`, `commit-guard`, `desktop-design-system`, `desktop-packaging-release`, `desktop-qa`, `electron-app-builder`, `env-audit`, `github-guard`, `guarded-change`, `i18n-checklist`, `java-maven-verify`, `merge-fix`, `node-workspace-release`, `release-guard`, `review-loop`, `security-review` |
...
| `node-workspace-check` | 检查 Node workspace 元数据、包管理器命令、lockfiles 和常见验证/发布脚本。 | `.\workflows\node-workspace-check\node-workspace-check.ps1 -Path .` | `./workflows/node-workspace-check/node-workspace-check.sh --path .` | `desktop-packaging-release`, `desktop-qa`, `electron-app-builder`, `node-workspace-release`, `release-guard` |
...
| `release-check` | 检查公开发布就绪信号，不 publish、tag、push、改版本或创建 artifact。 | `.\workflows\release-check\release-check.ps1 -Path .` | `./workflows/release-check/release-check.sh --path .` | `desktop-packaging-release`, `github-guard`, `release-guard`, `security-review` |
| `runtime-check` | 检查本地 agent runtime 前置条件，如 shell、PATH、包管理器命令、settings 和 hook wrappers。 | `.\workflows\runtime-check\runtime-check.ps1 -Path .` | `./workflows/runtime-check/runtime-check.sh --path .` | `desktop-qa`, `electron-app-builder`, `env-audit`, `runtime-repair` |
| `security-check` | 扫描 secret-like patterns 和 private path signals，不打印敏感值。 | `.\workflows\security-check\security-check.ps1 -Path .` | `./workflows/security-check/security-check.sh --path .` | `desktop-packaging-release`, `electron-app-builder`, `release-guard`, `security-review` |
