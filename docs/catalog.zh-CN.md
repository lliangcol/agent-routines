...
| `env-audit` | 跨平台环境、依赖、shell、PATH 和工具链审计。 | `preflight`, `gate-check` | 本地工具链或 shell 行为需要只读诊断。 |
...
|---|---|---|---|---|
| `archive-check` | 验证执行归档布局、必需文件、证据目录、产物目录和 front matter。 | `.\workflows\archive-check\archive-check.ps1 -Path .` | `./workflows/archive-check/archive-check.sh --path .` | `guarded-change`, `review-loop` |
| `commit-check` | 检查提交就绪度、分支状态、工作区脏状态、staged/untracked、git identity 和 diff checks。 | `.\workflows\commit-check\commit-check.ps1 -Path .` | `./workflows/commit-check/commit-check.sh --path .` | `commit-guard`, `guarded-change`, `review-loop` |
...
| `doc-check` | 聚合文档检查和可选安全自定义命令，并明确报告缺失 runtime。 | `.\workflows\doc-check\doc-check.ps1 -Path .` | `./workflows/doc-check/doc-check.sh --path .` | `pay-docs` |
| `drift-check` | 检查 knowledge roots、drift metadata、Markdown frontmatter 覆盖和本地 drift 工具可用性。 | `.\workflows\drift-check\drift-check.ps1 -Path .` | `./workflows/drift-check/drift-check.sh --path .` | `knowledge-drift`, `pay-docs` |
| `gate-check` | 运行安全通用门禁，如 git diff checks 和明确提供的非破坏性自定义命令。 | `.\workflows\gate-check\gate-check.ps1 -Path .` | `./workflows/gate-check/gate-check.sh --path .` | `guarded-change`, `review-loop`, `merge-fix`, `api-sync`, `env-audit` |
| `github-check` | 检查本地 GitHub Actions workflows 并推导候选 required checks，不修改远程设置。 | `.\workflows\github-check\github-check.ps1 -Path .` | `./workflows/github-check/github-check.sh --path .` | `github-guard`, `release-guard` |
| `governance-check` | 检查当前 checkout 的治理文件、agent 目录、验证脚本和 git 状态。 | `.\workflows\governance-check\governance-check.ps1 -Path .` | `./workflows/governance-check/governance-check.sh --path .` | `governance-audit`, `guarded-change` |
| `graph-check` | 检查 graph 和 MCP 就绪信号，不注册 MCP、不索引仓库、不安装 graph 工具、不上传代码。 | `.\workflows\graph-check\graph-check.ps1 -Path .` | `./workflows/graph-check/graph-check.sh --path .` | `graph-audit`, `env-audit`, `governance-audit` |
| `maven-check` | 检查 Java/Maven 就绪度、根 Maven 元数据、wrapper 和用户 Maven mirror 设置。 | `.\workflows\maven-check\maven-check.ps1 -Path .` | `./workflows/maven-check/maven-check.sh --path .` | `java-maven-verify`, `guarded-change` |
...
| `preflight` | 收集 repo path、branch、HEAD、dirty state、staged/unstaged/untracked 和 rule-file presence。 | `.\workflows\preflight\preflight.ps1 -Path .` | `./workflows/preflight/preflight.sh --path .` | `guarded-change`, `review-loop`, `api-sync`, `env-audit` |
| `release-check` | 检查公开发布就绪信号，不 publish、tag、push、改版本或创建 artifact。 | `.\workflows\release-check\release-check.ps1 -Path .` | `./workflows/release-check/release-check.sh --path .` | `release-guard`, `node-workspace-release`, `security-review` |
| `runtime-check` | 检查本地 agent runtime 前置条件，如 shell、PATH、包管理器命令、settings 和 hook wrappers。 | `.\workflows\runtime-check\runtime-check.ps1 -Path .` | `./workflows/runtime-check/runtime-check.sh --path .` | `runtime-repair`, `env-audit` |
| `security-check` | 扫描 secret-like patterns 和 private path signals，不打印敏感值。 | `.\workflows\security-check\security-check.ps1 -Path .` | `./workflows/security-check/security-check.sh --path .` | `security-review`, `release-guard`, `commit-guard` |
