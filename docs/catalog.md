...
| `env-audit` | Cross-platform environment, dependency, shell, PATH, and toolchain audits. | `preflight`, `gate-check` | Local toolchain or shell behavior needs readonly diagnosis. |
...
|---|---|---|---|---|
| `archive-check` | Validate execution archive layout, required files, evidence directories, artifacts directories, and front matter expectations. | `.\workflows\archive-check\archive-check.ps1 -Path .` | `./workflows/archive-check/archive-check.sh --path .` | `guarded-change`, `review-loop` |
| `commit-check` | Inspect commit readiness, branch state, worktree dirtiness, staged and untracked files, git identity, and diff checks. | `.\workflows\commit-check\commit-check.ps1 -Path .` | `./workflows/commit-check/commit-check.sh --path .` | `commit-guard`, `guarded-change`, `review-loop` |
...
| `doc-check` | Aggregate documentation checks and optional safe custom commands while reporting missing runtimes clearly. | `.\workflows\doc-check\doc-check.ps1 -Path .` | `./workflows/doc-check/doc-check.sh --path .` | `pay-docs` |
| `drift-check` | Inspect knowledge roots, drift metadata, Markdown frontmatter coverage, and local drift tool availability. | `.\workflows\drift-check\drift-check.ps1 -Path .` | `./workflows/drift-check/drift-check.sh --path .` | `knowledge-drift`, `pay-docs` |
| `gate-check` | Run safe common gates such as git diff checks and explicitly supplied non-destructive custom commands. | `.\workflows\gate-check\gate-check.ps1 -Path .` | `./workflows/gate-check/gate-check.sh --path .` | `guarded-change`, `review-loop`, `merge-fix`, `api-sync`, `env-audit` |
| `github-check` | Inspect local GitHub Actions workflows and derive candidate required checks without changing remote settings. | `.\workflows\github-check\github-check.ps1 -Path .` | `./workflows/github-check/github-check.sh --path .` | `github-guard`, `release-guard` |
| `governance-check` | Inspect current checkout governance files, agent directories, validation scripts, and git state. | `.\workflows\governance-check\governance-check.ps1 -Path .` | `./workflows/governance-check/governance-check.sh --path .` | `governance-audit`, `guarded-change` |
| `graph-check` | Inspect graph and MCP readiness signals without registering MCP servers, indexing repositories, or uploading code. | `.\workflows\graph-check\graph-check.ps1 -Path .` | `./workflows/graph-check/graph-check.sh --path .` | `graph-audit`, `env-audit`, `governance-audit` |
| `maven-check` | Inspect Java and Maven readiness, root Maven metadata, wrapper presence, and user Maven mirror settings. | `.\workflows\maven-check\maven-check.ps1 -Path .` | `./workflows/maven-check/maven-check.sh --path .` | `java-maven-verify`, `guarded-change` |
...
| `node-workspace-check` | Inspect Node workspace metadata, package-manager commands, lockfiles, and validation or release scripts. | `.\workflows\node-workspace-check\node-workspace-check.ps1 -Path .` | `./workflows/node-workspace-check/node-workspace-check.sh --path .` | `node-workspace-release`, `guarded-change` |
| `preflight` | Collect repository path, branch, HEAD, dirty state, staged/unstaged/untracked signals, and rule-file presence. | `.\workflows\preflight\preflight.ps1 -Path .` | `./workflows/preflight/preflight.sh --path .` | `guarded-change`, `review-loop`, `api-sync`, `env-audit` |
| `release-check` | Inspect public release readiness signals without publishing, tagging, pushing, mutating versions, or creating artifacts. | `.\workflows\release-check\release-check.ps1 -Path .` | `./workflows/release-check/release-check.sh --path .` | `release-guard`, `node-workspace-release`, `security-review` |
| `runtime-check` | Inspect local agent runtime prerequisites such as shell, PATH, package-manager commands, settings, and hook wrappers. | `.\workflows\runtime-check\runtime-check.ps1 -Path .` | `./workflows/runtime-check/runtime-check.sh --path .` | `runtime-repair`, `env-audit` |
| `security-check` | Scan local files for secret-like patterns and private path signals without printing sensitive values. | `.\workflows\security-check\security-check.ps1 -Path .` | `./workflows/security-check/security-check.sh --path .` | `security-review`, `release-guard`, `commit-guard` |
